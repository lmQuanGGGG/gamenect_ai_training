...
## 📉 CÂU HỎI 27: GIẢI THÍCH MA TRẬN NHẦM LẪN (CONFUSION MATRIX) VÀ SỰ ĐÁNH ĐỔI PRECISION/RECALL

👨‍🏫 **Hội đồng hỏi:** *"Trong kết quả test của em có ma trận: [ TN: 495 | FP: 382 ] và [ FN: 66 | TP: 1060 ]. Từ đó tính ra Precision chỉ có 73.5%. Tại sao Precision thấp vậy mà em lại đánh giá là mô hình tốt?"*

* 🛡️ **Hướng trả lời:**
"Dạ thưa thầy/cô, trong các hệ thống Gợi ý (Recommendation System), đặc biệt là ứng dụng ghép đôi, mục tiêu tối thượng không phải là Precision (Độ chính xác tuyệt đối) mà là **Recall (Độ phủ)**.
Từ ma trận trên, nhóm tính được **Recall = TP / (TP + FN) = 1060 / (1060 + 66) ≈ 94.1%**.
Sự đánh đổi (Trade-off) ở đây là:
- Việc gợi ý nhầm một vài người không hợp (**False Positive - 382 mẫu**): Hoàn toàn có thể chấp nhận được vì người dùng chỉ mất 1 giây để lướt bỏ qua (Pass).
- Việc bỏ sót những người thực sự rất hợp (**False Negative - chỉ có 66 mẫu**): Đây là lỗi nghiêm trọng nhất làm giảm trải nghiệm cốt lõi của mạng xã hội. 
Nếu tụi em cố tình đẩy Precision lên >95%, thuật toán sẽ trở nên quá khắt khe, dẫn đến việc người dùng không có ai để quẹt cả. Vì vậy, ma trận với Recall cực cao (94%) và Precision vừa đủ (73.5%) là **điểm cân bằng lý tưởng nhất (Optimal Threshold)** cho môi trường thực tế ạ."

---

## 🌳 CÂU HỎI 28: KIỂM SOÁT OVERFITTING CHO GRADIENT BOOSTING

👨‍🏫 **Hội đồng hỏi:** *"Thuật toán Gradient Boosting rất dễ bị Overfitting (Học vẹt) khi tăng số lượng cây. Em đã làm gì để chống lại hiện tượng này khi setup Hyperparameters?"*

* 🛡️ **Hướng trả lời:**
"Dạ đúng là Gradient Boosting rất nhạy cảm với Overfitting. Để trị triệt để căn bệnh này, nhóm em đã kết hợp 3 kỹ thuật song song (Regularization):
1. **Giảm Learning Rate (0.05):** Thay vì để tốc độ học nhanh (0.1), nhóm giảm xuống 0.05 để mô hình đi từng bước nhỏ, bù đắp lại việc số lượng cây lớn (`n_estimators = 300`).
2. **Giới hạn độ sâu của cây (`max_depth = 6`):** Khóa không cho cây phát triển quá sâu để tránh nó 'nhớ' những dữ liệu nhiễu (noise) của tập Train.
3. **Áp dụng Stochastic Gradient Boosting (`subsample = 0.8`):** Đây là kỹ thuật cốt lõi. Mỗi cây quyết định chỉ được nhìn thấy ngẫu nhiên 80% dữ liệu. Việc thêm yếu tố ngẫu nhiên (Randomness) này giúp mô hình tăng tính khái quát hóa (Generalization) khi gặp tập Test mới hoàn toàn ạ."

---

## 🧠 CÂU HỎI 29: AI EXPLAINABILITY (KHẢ NĂNG GIẢI THÍCH CỦA AI)

👨‍🏫 **Hội đồng hỏi:** *"Máy học thường bị gọi là 'Hộp đen' (Black box). Khi AI ghép 2 người với nhau, làm sao em chứng minh được nó đang quyết định dựa trên điều gì?"*

* 🛡️ **Hướng trả lời:**
"Dạ để 'mở nắp' chiếc hộp đen này, đồ án sử dụng thư viện **SHAP (SHapley Additive exPlanations)**. Công cụ này áp dụng lý thuyết trò chơi (Game Theory) để tính toán mức độ đóng góp của từng biến số vào kết quả cuối cùng.
